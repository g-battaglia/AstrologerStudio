    ...
    try {
      ...
      if (exportOptions.includeChartWheel) {
        ...
        try {
          ...
          console.warn('Failed to regenerate transit chart with light theme:', err)
          if (chartWheelHtml) {
      ...
      const subjectName = natalChartData.subject.name.replace(/[^a-zA-Z0-9]/g, '_')
      const dateStr = new Date().toISOString().split('T')[0]
      link.download = `${subjectName}_transit_chart_${dateStr}.pdf`
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Export Transit Chart as PDF</DialogTitle>
          <DialogDescription>Configure what to include in your transit report.</DialogDescription>
        </DialogHeader>
