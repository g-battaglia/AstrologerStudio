...
import { SynastryChartPDF } from './SynastryChartPDF'
    ...
    try {
      ...
      if (exportOptions.includeChartWheel) {
        ...
        try {
          ...
          console.warn('Failed to regenerate synastry chart with light theme:', err)
          if (chartWheelHtml) {
      ...
      const name1 = subject1ChartData.subject.name.replace(/[^a-zA-Z0-9]/g, '_')
      const name2 = subject2ChartData.subject.name.replace(/[^a-zA-Z0-9]/g, '_')
      const dateStr = new Date().toISOString().split('T')[0]
      link.download = `${name1}_${name2}_synastry_${dateStr}.pdf`
      ...
      trackPdfExport('synastry').catch(() => {})

      toast.success('Synastry PDF exported successfully!')
      setOpen(false)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Export Synastry Chart as PDF</DialogTitle>
          <DialogDescription>Configure what to include in your synastry report.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="include-rscore" className="cursor-pointer">
                Relationship Score
              </Label>
              <Switch
                id="include-rscore"
