...
export interface SolarReturnPDFProps {
...
/**
 * SolarReturnPDF Document
 *
 * PDF document for Solar Return charts.
 * Supports both single wheel (return only) and dual wheel (natal + return).
 */
export function SolarReturnPDF({
  chartData,
  aspects,
  chartWheelImage,
  notes,
  ...
  const reportTitle = 'Solar Return Report'
    ...
    >
      ...
      <Page size="A4" style={styles.page}>
        ...
        {options.includeChartWheel && chartWheelImage && (
          <PDFChartWheel imageData={chartWheelImage} caption={`${subjectName}'s Solar Return`} />
        )}
      ...
      {/* Page 3: Solar Return Planetary Positions */}
      {options.includePlanets && (
        <Page size="A4" style={styles.page} wrap>
          <PDFHeader
            ...
            logoData={branding.logoData}
            brandingText={branding.text}
            reportTitle="Solar Return Positions"
          />

          <PDFPlanetaryTable subject={returnSubject} title="Solar Return Planetary Positions" />

          <PDFFooter showFooter={branding.showFooter} footerText={branding.footerText} />
        </Page>
      )}

      {/* Page 4: Solar Return House Cusps */}
      {options.includeHouses && (
        <Page size="A4" style={styles.page}>
          <PDFHeader
            ...
            logoData={branding.logoData}
            brandingText={branding.text}
            reportTitle="Solar Return Houses"
          />

          <PDFHousesTable subject={returnSubject} title="Solar Return House Cusps" />
      ...
      {isActualDual && houseComparison && houseComparison.second_points_in_first_houses && (
        <Page size="A4" style={styles.page} wrap>
          ...
          <PDFHouseOverlayTable
            ...
            title="Solar Return Planets in Natal Houses"
            description={`Where the Solar Return planets fall within ${subjectName}'s natal house system`}
          />
      ...
      {options.includeAspects && aspects.length > 0 && (
        <Page size="A4" style={styles.page} wrap>
          <PDFHeader
            ...
            logoData={branding.logoData}
            brandingText={branding.text}
            reportTitle="Solar Return Aspects"
          />

          <PDFAspectsSection
            aspects={aspects}
            title={isActualDual ? 'Solar Return to Natal Aspects' : 'Solar Return Aspects'}
            maxAspects={80}
          />
      ...
      {options.includeInterpretation && notes && notes.trim().length > 0 && (
        <Page size="A4" style={styles.page} wrap>
          ...
          <PDFNotesSection notes={notes} title="Solar Return Interpretation" />
